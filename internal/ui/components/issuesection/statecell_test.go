...
// glyph-presence assertion for issues: open/closed both render their
// section.StateCell glyph in the "state" column.
func TestBuildRowStateCellHasGlyphPerState(t *testing.T) {
	cases := []struct {
		...
		{"open", data.Issue{Number: 1, State: "open"}, icons.Open},
		{"closed", data.Issue{Number: 2, State: "closed"}, icons.Closed},
	}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			m.SetLastFetchID("t")
			next, _ := m.Update(SectionIssuesFetchedMsg{Rows: []data.Issue{c.issue}, TotalCount: 1, TaskId: "t"})
			m = next.(*Model)
			row := m.BuildRows()[0]
...
func TestBuildRowUsesConfiguredASCIIIconSet(t *testing.T) {
	...
	next, _ := m.Update(SectionIssuesFetchedMsg{
		Rows:       []data.Issue{{Number: 1, State: "open"}},
