...
- [ ] **Step 5: tmux verification** (per ground rules): capture at full size, then `tmux resize-pane -x 70 -y 18` (preview auto-collapses), `-x 39 -y 9` (too-small notice), restore. Confirm borders join cleanly (no gaps/overdraw), all five views, search open state.
